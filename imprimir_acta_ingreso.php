...
<head>
    ...
    <title>Acta Informativa de Ingreso - #<?= $solicitud['id'] ?></title>
...
<body>
    ...
    <!-- Documento -->
    <div class="document-container border-t-8 border-blue-500 avoid-break">
        ...
        <div class="text-center border-b-2 border-slate-200 pb-4 mb-6">
            ...
            <h1 class="text-3xl font-bold text-slate-900 uppercase">Acta Informativa de Ingreso</h1>
            ...
            <div class="mt-2 inline-block bg-blue-100 text-blue-800 text-xs font-bold px-3 py-1 rounded uppercase">Copia
                Informativa - No Válida como Contrato</div>
        ...
        <div class="mb-6 avoid-break">
            <h2 class="text-lg font-bold text-slate-800 border-l-4 border-blue-500 pl-3 mb-3 uppercase">1. Datos del
                Colaborador</h2>
            <div class="grid grid-cols-2 gap-x-8 gap-y-4 bg-slate-50 p-4 rounded-lg">
                ...
                <div>
                    <span class="text-xs font-bold text-slate-500 uppercase block">Fecha Solicitud</span>
                    <span
                        class="text-sm font-semibold text-slate-800"><?= date('d/m/Y', strtotime($solicitud['fecha_solicitud'])) ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6 avoid-break">
            <h2 class="text-lg font-bold text-slate-800 border-l-4 border-blue-500 pl-3 mb-3 uppercase">2.
                Requerimientos Tecnológicos</h2>
            <table class="w-full text-sm mb-2 text-left">
                <thead class="bg-slate-100 text-slate-600">
                    <tr>
                        <th class="py-2 px-3 font-bold">Servicio</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr>
                        <td class="py-2 px-3 font-bold text-slate-600">Licencias (M365)</td>
                        <td class="py-2 px-3 text-center font-bold"><?= $solicitud['disponibilidad_licencias'] ?></td>
                        <td class="py-2 px-3 text-slate-500 italic">
                            <?= htmlspecialchars($solicitud['detalle_licencias'] ?? '-') ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-bold text-slate-600">Crear Correo</td>
                        <td class="py-2 px-3 text-center font-bold"><?= $solicitud['correo_nuevo'] ?></td>
                        <td class="py-2 px-3 text-slate-500 italic">
                            <?= htmlspecialchars($solicitud['direccion_correo'] ?? '-') ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-bold text-slate-600">Mostrar Remitente</td>
                        <td class="py-2 px-3 text-center font-bold"><?= $solicitud['remitente_mostrar'] ?></td>
                        <td class="py-2 px-3 text-slate-500 italic">
                            <?= htmlspecialchars($solicitud['detalle_remitente'] ?? '-') ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-bold text-slate-600">Respaldo Nube</td>
                        <td class="py-2 px-3 text-center font-bold"><?= $solicitud['respaldo_nube'] ?></td>
                        <td class="py-2 px-3 text-slate-500 italic">
                            <?= htmlspecialchars($solicitud['detalle_respaldo'] ?? '-') ?>
                        </td>
                    </tr>
                    <tr>
                        <td class="py-2 px-3 font-bold text-slate-600">Reenvío Correos</td>
                        <td class="py-2 px-3 text-center font-bold"><?= $solicitud['reenvios_correo'] ?></td>
                        <td class="py-2 px-3 text-slate-500 italic">
        ...
        <div class="mb-6 avoid-break">
            <h2 class="text-lg font-bold text-slate-800 border-l-4 border-blue-500 pl-3 mb-3 uppercase">3. Asignación de
                Activos</h2>
        ...
        <?php if (!empty($solicitud['otras_indicaciones'])): ?>
            <div class="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg avoid-break">
                <h3 class="font-bold text-yellow-800 text-sm uppercase mb-2">Indicaciones Adicionales</h3>

        ...
        <div class="flex items-center gap-4">
            ...
            if (isset($pdo) && isset($usuario_id)) {
                try {
                    ...
                    // Obtener últimas 5
                    $stmt_notifs = $pdo->prepare("SELECT * FROM notificaciones WHERE usuario_id = ? ORDER BY created_at DESC LIMIT 5");
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" onclick="document.getElementById('notif-dropdown').classList.toggle('hidden');"
                ...
                <div id="notif-dropdown" class="hidden absolute right-0 mt-3 w-80 bg-white rounded-xl shadow-xl border border-slate-100 z-50 overflow-hidden">
                    <div class="p-3 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                        ...
                        <?php if ($count_noleidas > 0): ?>
                            <span id="notif-count-text" class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-medium cursor-pointer hover:bg-blue-200 transition-colors" onclick="marcarNotificacion('todas')">
                                Marcar todas leídas
                            </span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="max-h-[300px] overflow-y-auto">
                        ...
                        <?php else: ?>
                            <?php foreach ($notificaciones as $n): ?>
                                <div onclick="marcarNotificacion(<?= $n['id'] ?>, this)" 
                                     class="notif-item p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer relative group <?= $n['leida'] ? 'opacity-60' : '' ?>">
                                    <?php if (!$n['leida']): ?>
                                    ...
                                    <div class="flex gap-3">
                                        <div class="mt-1">
                                            ...
                                            $color = 'text-blue-500';
                                            if ($n['tipo'] == 'success') { $icon = 'ri-checkbox-circle-line'; $color = 'text-emerald-500'; }
                                            if ($n['tipo'] == 'warning') { $icon = 'ri-alert-line'; $color = 'text-amber-500'; }
                                            if ($n['tipo'] == 'error') { $icon = 'ri-error-warning-line'; $color = 'text-red-500'; }
                                            ?>
                                            <i class="<?= $icon ?> <?= $color ?> text-lg"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-sm font-semibold text-slate-800 mb-0.5"><?= htmlspecialchars($n['titulo']) ?></h4>
                                            <p class="text-xs text-slate-500 leading-relaxed"><?= htmlspecialchars($n['mensaje']) ?></p>
                    ...
                    <div class="p-2 border-t border-slate-50 bg-slate-50 text-center">
                        <a href="index.php?view=notificaciones" class="text-xs text-blue-600 font-medium hover:text-blue-700">Ver todas</a>
                    </div>
                </div>
            </div>
            ...
            <script>
            function marcarNotificacion(id, elemento) {
                // Si es individual y ya está leída (tiene opacidad), no hacemos nada visualmente crítico, pero igual podemos mandar
                ...
                .then(data => {
                    if(data.status === 'success') {
                        if (id === 'todas') {
                            ...
                            const btnTodas = document.getElementById('notif-count-text');
                            if(btnTodas) btnTodas.style.display = 'none';

                        } else {
                            // Marcar individual
                            if(elemento) {
                                elemento.classList.add('opacity-60');
                                const ind = elemento.querySelector('.notif-indicator');
                                if(ind) ind.remove();
                            }
                            // Opcional: bajar contador o checar si quedan
                        }
                    }
                });
            }
